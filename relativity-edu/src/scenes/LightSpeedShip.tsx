  ...
  const { velocities, geometry } = useMemo(() => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      if (colorChoice < 0.3) {
        ...
        colors[i3] = 1.0; colors[i3 + 1] = 0.8; colors[i3 + 2] = 0.3
      }
    }
    ...
    <group>
      <group ref={shipRef} position={[0, 0, 0]}>
        ...
        <mesh position={[0, -0.3, -0.5]}>
          ...
          <meshStandardMaterial color="#3a8eff" emissive="#1a4eff" emissiveIntensity={0.3} metalness={0.7} roughness={0.3} />
        </mesh>
        <pointLight position={[0, 0, 1]} color="#4a9eff" intensity={2} distance={5} />
      </group>
      <points ref={particlesRef} geometry={geometry}>
        <pointsMaterial size={0.1} vertexColors transparent opacity={0.8} sizeAttenuation blending={THREE.AdditiveBlending} />
      </points>
    </group>
  )
}
